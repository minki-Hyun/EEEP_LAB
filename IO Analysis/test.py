...
# 기본부문이 소분류속 몇개인지 세기
file = pd.read_excel(url_home, sheet_name="상품분류표",header = 1, usecols=["기본부문(381)","소분류(165)"])
file = file.iloc[1:,:]
...
io_mat_a = pd.read_excel(url1_home,sheet_name="A표_국산거래표(생산자)", header=5, index_col=1, skipfooter=1)
...
io_mat_employ = pd.read_excel(url1_home,sheet_name="취업자수 및 피용자수(상품)",index_col=0,skipfooter=1)
flag_em = io_mat_employ.to_numpy() / np.array(li1).reshape(len(li1),1)
print(len(io_mat_a.reshape(len(io_mat_a),1)))
print()
print(flag_em[0])
io_mat_b = np.array([])
a0 = 0
